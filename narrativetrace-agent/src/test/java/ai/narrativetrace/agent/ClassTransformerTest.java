...
class ClassTransformerTest {
  ...
  void transformedClassCapturesParameterNamesAndValues() throws Exception {
    ...
    assertThat(root.signature().parameters())
        .containsExactly(
            new ParameterCapture("a", "3", false), new ParameterCapture("b", "4", false));
  }
  ...
  void methodWithoutParameterNamesUsesSimpleEnterMethod() throws Exception {
    ...
    var loader =
        new ByteArrayClassLoader(getClass().getClassLoader(), transformed, "test.NoParams");
  ...
  void staticMethodParametersStartAtSlotZero() throws Exception {
    ...
    assertThat(root.signature().parameters())
        .containsExactly(
            new ParameterCapture("a", "10", false), new ParameterCapture("b", "20", false));
  }
  ...
  void partialParamNamesFallBackToArgIndex() throws Exception {
    ...
    var loader =
        new ByteArrayClassLoader(getClass().getClassLoader(), transformed, "test.PartialNames");
  ...
  void wideParamsResolvedViaLocalVariableTable() throws Exception {
    // Generate a class with long/double params, debug info (LocalVariableTable) but no
    // MethodParameters
    var cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES);
    ...
    var loader =
        new ByteArrayClassLoader(getClass().getClassLoader(), transformed, "test.WideLocals");
  ...
  void onErrorWithoutParameterNamesDoesNotCrash() throws Exception {
    // Generate a class with @OnError annotation but no parameter names
    // (no MethodParameters attribute, no LocalVariableTable).
    // This must not NPE in visitMaxs when storedLocals is null.
    var cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES);
    cw.visit(
        Opcodes.V17, Opcodes.ACC_PUBLIC, "test/OnErrorNoParams", null, "java/lang/Object", null);
    var init = cw.visitMethod(Opcodes.ACC_PUBLIC, "<init>", "()V", null, null);
    init.visitCode();
    init.visitVarInsn(Opcodes.ALOAD, 0);
    init.visitMethodInsn(Opcodes.INVOKESPECIAL, "java/lang/Object", "<init>", "()V", false);
    ...
    // Method with @OnError but no param names
    var mv = cw.visitMethod(Opcodes.ACC_PUBLIC, "process", "(I)V", null, null);
    var av = mv.visitAnnotation("Lai/narrativetrace/core/annotation/OnError;", true);
    ...
    var transformed = ClassTransformer.transform(originalBytes, "test/OnErrorNoParams");

    var loader =
        new ByteArrayClassLoader(getClass().getClassLoader(), transformed, "test.OnErrorNoParams");
    var clazz = loader.loadClass("test.OnErrorNoParams");
    var instance = clazz.getDeclaredConstructor().newInstance();

    try {
      clazz.getMethod("process", int.class).invoke(instance, 42);
    } catch (Exception e) {
      // expected
    }
    ...
    // Parameters were not captured (no names available)
    assertThat(tree.roots().get(0).signature().parameters()).isEmpty();
  }
