...
class NestedTraceTest {
    ...
    void transformedClassProducesCorrectNestedTrace() throws Exception {
        ...
        var calcBytes = getClass().getClassLoader()
                .getResourceAsStream("ai/narrativetrace/agent/sample/Calculator.class")
                .readAllBytes();
        var procBytes = getClass().getClassLoader()
                .getResourceAsStream("ai/narrativetrace/agent/sample/OrderProcessor.class")
                .readAllBytes();

        var transformedCalc = ClassTransformer.transform(calcBytes,
                "ai/narrativetrace/agent/sample/Calculator");
        var transformedProc = ClassTransformer.transform(procBytes,
                "ai/narrativetrace/agent/sample/OrderProcessor");
        ...
        assertThat(((TraceOutcome.Returned) root.children().get(0).outcome()).renderedValue()).isEqualTo("15");
    }
}
