...
class MethodMetadataCollectorTest {
    ...
    void collectsParameterNamesFromCalculator() throws Exception {
        var bytes = getClass().getClassLoader()
    ...
    void collectsNotTracedAnnotation() throws Exception {
        ...
        var metadata = collector.getMetadata();
        var loginMeta = metadata.get(MethodMetadataCollector.key("login",
    ...
    void collectsNarratedTemplate() throws Exception {
        ...
        var metadata = collector.getMetadata();
        var calcMeta = metadata.get(MethodMetadataCollector.key("calculatePrice",
    ...
    void collectsOnErrorAnnotations() throws Exception {
        ...
        var metadata = collector.getMetadata();
        var transferMeta = metadata.get(MethodMetadataCollector.key("transfer",
                "(Ljava/lang/String;Ljava/lang/String;I)V"));
        assertThat(transferMeta).isNotNull();
        assertThat(transferMeta.onErrors()).hasSize(2);
        assertThat(transferMeta.onErrors()[0].template()).isEqualTo("Transfer failed for amount {amount}");
        assertThat(transferMeta.onErrors()[0].exceptionDescriptor()).isEqualTo("Ljava/lang/IllegalArgumentException;");
        assertThat(transferMeta.onErrors()[1].template()).isEqualTo("Transfer error for amount {amount}");
