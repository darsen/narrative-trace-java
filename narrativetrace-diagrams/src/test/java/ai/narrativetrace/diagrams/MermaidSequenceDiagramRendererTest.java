...
class MermaidSequenceDiagramRendererTest {
    ...
    void rendersNestedCallsWithCorrectParticipantOrderingAndFlow() {
        var inventoryCall = new TraceNode(
                new MethodSignature("InventoryService", "reserveStock", List.of(
                        new ParameterCapture("sku", "\"SKU-1\"", false)
                )),
                List.of(),
                new TraceOutcome.Returned("true"),
                5_000_000L
        );
        var paymentCall = new TraceNode(
    ...
    void rendersParticipantWithDotsInQuotes() {
        ...
        assertThat(diagram).contains("\"com.example.OrderService\"->>\"com.example.OrderService\": placeOrder()");
    }
