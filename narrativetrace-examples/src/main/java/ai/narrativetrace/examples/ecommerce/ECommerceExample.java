...
/**
 * E-commerce example using Spring {@code @EnableNarrativeTrace} auto-proxying and {@code @Async}
 * cross-thread propagation via the Micrometer bridge.
 *
 * <p>Correlation: in a Spring Boot app, Micrometer Tracing automatically puts a {@code traceId} in
 * MDC for every inbound HTTP request. Here we simulate that with sequential IDs since there is no
 * inbound request. The {@link ContextPropagatingTaskDecorator} propagates both MDC and narrative
 * context to async threads, so log lines from both threads share the same trace ID.
 */
