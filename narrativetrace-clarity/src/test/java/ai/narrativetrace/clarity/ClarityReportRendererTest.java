...
class ClarityReportRendererTest {
    ...
    void rendersOverallScoreTable() {
        ...
        assertThat(markdown).contains("| Method Names | 1.00 | 0.30 |");
        assertThat(markdown).contains("| Class Names | 0.90 | 0.20 |");
        assertThat(markdown).contains("| Parameter Names | 0.70 | 0.25 |");
        assertThat(markdown).contains("| Structural | 0.80 | 0.15 |");
    ...
    void rendersIssuesTableWithSuggestions() {
        var issues = List.of(
                new ClarityIssue("method-name", "Service.process", "Use a domain-specific verb+noun",
                        ClarityIssue.Severity.HIGH, 1, 3.0),
                new ClarityIssue("param-name", "data", "Use a domain-specific name",
                        ClarityIssue.Severity.HIGH, 1, 3.0)
        );
    ...
    void rendersSuiteReportWithLowestClarityScenarios() {
        ...
        var lowClarity = new ClarityResult(0.25, 0.0, 0.0, 0.0, 1.0, 0.7, List.of(
                new ClarityIssue("method-name", "Service.process", "Use a domain-specific verb+noun",
                        ClarityIssue.Severity.HIGH, 1, 3.0)
        ));
        var results = Map.of(
    ...
    void rendersSeverityColumn() {
        var issues = List.of(
                new ClarityIssue("method-name", "Service.process", "Fix it",
                        ClarityIssue.Severity.HIGH, 1, 3.0)
        );
        var result = new ClarityResult(0.4, 0.0, 0.0, 0.0, 1.0, 0.7, issues);
    ...
    void rendersIssuesSortedByImpact() {
        var issues = List.of(
                new ClarityIssue("param-name", "count", "low",
                        ClarityIssue.Severity.LOW, 1, 1.0),
                new ClarityIssue("method-name", "process", "high",
                        ClarityIssue.Severity.HIGH, 3, 9.0)
        );
    ...
    void rendersOccurrencesCount() {
        var issues = List.of(
                new ClarityIssue("param-name", "data", "fix",
                        ClarityIssue.Severity.HIGH, 3, 9.0)
        );
        var result = new ClarityResult(0.3, 0.0, 0.0, 0.0, 1.0, 0.7, issues);
    ...
    void suiteReportIncludesCohesion() {
        var lowClarity = new ClarityResult(0.25, 0.0, 0.0, 0.0, 1.0, 0.3, List.of(
                new ClarityIssue("method-name", "Service.process", "fix",
                        ClarityIssue.Severity.HIGH, 1, 3.0)
        ));
