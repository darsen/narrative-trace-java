...
class Slf4jNarrativeContextTest {
  ...
  @Test
  void depthRestartsAtOneAfterResetWithUnbalancedStack() {
    var signature = new MethodSignature("Svc", "method", List.of());
    context.enterMethod(signature);
    context.enterMethod(signature);
    // No exits — simulate reset mid-request (e.g., servlet filter reset)
    ...
    var mdc = appender.list.get(0).getMDCPropertyMap();
    assertThat(mdc).containsEntry("nt.depth", "1");
  }
  ...
  void errorContextAppearsInExceptionLog() {
    ...
    context.exitMethodWithException(
        new RuntimeException("declined"), "Payment declined for customer C-123");
  ...
  void isActiveDelegatesToWrappedContext() {
    ...
    var noopContext =
        new Slf4jNarrativeContext(ai.narrativetrace.core.context.NoopNarrativeContext.INSTANCE);
