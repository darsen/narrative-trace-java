...
public final class ThreadLocalNarrativeContext implements NarrativeContext {
    ...
    private MethodSignature suppressParameterValues(MethodSignature signature) {
        var suppressed = signature.parameters().stream()
                .map(p -> new ParameterCapture(p.name(), "", p.redacted()))
                .toList();
        return new MethodSignature(signature.className(), signature.methodName(), suppressed,
                signature.narration(), signature.errorContext());
    }
    ...
    private static final class TraceStack {
        ...
        void pop(TraceOutcome outcome, String errorContext) {
            ...
            if (errorContext != null) {
                signature = new MethodSignature(signature.className(), signature.methodName(),
                        signature.parameters(), signature.narration(), errorContext);
            }
