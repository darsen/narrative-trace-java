...
class TemplateWarningCollectorTest {
    ...
    void detectsUnresolvedPlaceholderInErrorContext() {
        var sig = new MethodSignature("OrderService", "placeOrder",
                List.of(), null, "Failed for {orderId}");
        var node = new TraceNode(sig, List.of(), new TraceOutcome.Returned("\"ok\""));
        var tree = new DefaultTraceTree(List.of(node));
    ...
    void detectsUnresolvedInChildNodes() {
        var childSig = new MethodSignature("InventoryService", "reserve",
                List.of(), "Reserving {sku}", null);
    ...
    void formatRendersReadableOutput() {
        var warnings = List.of(
                new TemplateWarningCollector.TemplateWarning("OrderService", "placeOrder", "custmerId", "narration"));
