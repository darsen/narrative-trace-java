...
class ThreadLocalNarrativeContextTest {
    ...
    void enterExitSingleMethodProducesOneRootNode() {
        ...
        var signature = new MethodSignature("OrderService", "placeOrder",
    ...
    void defaultConstructorUsesDetailLevel() {
        ...
        var tree = context.captureTrace();
        assertThat(tree.roots()).hasSize(1);
        assertThat(tree.roots().get(0).signature().parameters().get(0).renderedValue()).isEqualTo("\"C-123\"");
    }
    ...
    void downgradeToOffDuringInFlightMethodDoesNotLeakFrame() {
        ...
        var secondRoots = tree.roots().stream()
                .filter(r -> r.signature().methodName().equals("second"))
                .toList();
    ...
    void errorContextFlowsToTraceNodeSignatureOnException() {
        var context = new ThreadLocalNarrativeContext();
        context.enterMethod(new MethodSignature("PaymentService", "charge", List.of()));
        context.exitMethodWithException(new RuntimeException("declined"), "Payment was declined for customer C-123");

        var tree = context.captureTrace();
        var root = tree.roots().get(0);
        assertThat(root.signature().errorContext()).isEqualTo("Payment was declined for customer C-123");
    }
        ...
        try {
            var childTree = executor.submit(() -> {
                try (var scope = snapshot.activate()) {
                    context.enterMethod(new MethodSignature("ChildService", "childMethod", List.of()));
    ...
    void scopeRestoresPreviousStateOnPooledThread() throws Exception {
        ...
        try {
            ...
            executor.submit(() -> {
                context.enterMethod(new MethodSignature("ExistingService", "existingMethod", List.of()));
            ...
            executor.submit(() -> {
                try (var scope = snapshot.activate()) {
                    ...
                    assertThat(duringScope.roots().get(0).signature().className()).isEqualTo("NewService");
                }
                ...
                assertThat(afterScope.roots().get(0).signature().className()).isEqualTo("ExistingService");
            }).get();
        } finally {
            executor.shutdown();
        }
    ...
    void wrapRunnableActivatesScopeAndCloses() throws Exception {
        ...
        ExecutorService executor = Executors.newSingleThreadExecutor();
        try {
            var wrappedTree = executor.submit(() -> {
                var wrapped = snapshot.wrap((Runnable) () -> {
                    context.enterMethod(new MethodSignature("WrappedService", "run", List.of()));
